...
// Install checks
//
// The server's compat.py runs the checks (a pack too old, a fork under
// the same node name, a pack missing); this only shows what it found.
// Shown IN PLACE OF the timeline's controls, not beside them: a panel
// that can be scrolled past is a panel that will be, and the run would
// then stop on the same problems with a wall of text in the queue. The
// node refusing to be a timeline until the install is right is the one
// message nobody misses.
// ---------------------------------------------------------------------

/** The problem list as a panel. Text only: every string is data. */
function tlCompatPanel(problems, onRecheck) {
    const P = themePalette();
    const panel = document.createElement("div");
    Object.assign(panel.style, {
        display: "flex", flexDirection: "column", gap: "10px",
        padding: "10px 12px", overflowY: "auto", minHeight: "0", flex: "1",
        border: "1px solid rgba(220,80,80,0.6)", borderRadius: "6px",
        background: "rgba(220,80,80,0.08)", font: "12px sans-serif",
        color: P.text, boxSizing: "border-box",
    });
    const head = document.createElement("div");
    Object.assign(head.style, { font: "600 14px sans-serif" });
    head.textContent = "This workflow cannot run on this install yet";
    const sub = document.createElement("div");
    sub.style.color = P.sub;
    sub.textContent = (problems.length === 1 ? "One thing" : problems.length
                       + " things") + " to fix, then restart ComfyUI and "
                      + "reload the workflow:";
    ...
    problems.forEach((p, i) => {
        ...
        Object.assign(item.style, {
            ...
            paddingLeft: "10px", borderLeft: "3px solid rgba(220,80,80,0.6)",
        });
        const title = document.createElement("div");
        title.style.fontWeight = "600";
        title.textContent = (i + 1) + ". " + String(p.title ?? "");
        const detail = document.createElement("div");
        detail.textContent = String(p.detail ?? "");
        const fix = document.createElement("div");
        const fixLabel = document.createElement("b");
        fixLabel.textContent = "Fix: ";
        fix.append(fixLabel, document.createTextNode(String(p.fix ?? "")));
        item.append(title, detail, fix);
        ...
        if (links.length) {
            ...
            for (const link of links) {
                ...
                // http(s) only: a URL is data from the server like the
                // rest, and an anchor is the one place text becomes an
                // action
                ...
                a.textContent = String(link?.label ?? url) + " ↗";
                Object.assign(a.style, { color: "#7ab8ff",
    ...
    if (onRecheck) {
        ...
        Object.assign(again.style, {
            alignSelf: "flex-start", cursor: "pointer", padding: "3px 10px",
            borderRadius: "4px", border: "1px solid " + P.edge,
            background: P.rest, color: P.text, font: "11px sans-serif",
        });
...
/**
 * Ask the server, and swap the timeline's controls for the problem list
 * while there is one. `children` are the controls; they are hidden, not
 * removed, so the timeline is intact when the install is fixed and
 * "Check again" clears the panel. A server that cannot be asked at all
 * is left alone: the checks are a courtesy, and the node still refuses
 * at run time on its own.
 */
async function tlApplyCompat(container, children, fetchProblems) {
    let problems;
    try {
    ...
    const old = container.__tlCompatPanel;
    if (old) old.remove();
    container.__tlCompatPanel = null;
    if (!problems.length) {
        for (const c of children) {
            // each child back to what IT had: the progress bar starts
            // hidden and must come back hidden
            if (c.__tlDisplay !== undefined) c.style.display = c.__tlDisplay;
            delete c.__tlDisplay;
        }
        return [];
    }
    for (const c of children) {
        if (c.__tlDisplay === undefined) c.__tlDisplay = c.style.display;
        c.style.display = "none";
    }
    const panel = tlCompatPanel(problems, () => {
        void tlApplyCompat(container, children, fetchProblems);
    });
    container.__tlCompatPanel = panel;
    container.appendChild(panel);
    return problems;
}

async function tlFetchCompat() {
    const r = await api.fetchApi("/obvpm/h3/compat");
...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData) {
        ...
        function buildTimeline(node, result) {
            ...
            // Before anything else is shown: while the install cannot
            // run the workflow, the problems and their fixes stand
            // where the controls would be (compat.py decides).
            void tlApplyCompat(container,
                               [videoWrap, header, progress.el, timelineWrap,
                                cutRow, infoRow, nextRunRow],
                               tlFetchCompat);
