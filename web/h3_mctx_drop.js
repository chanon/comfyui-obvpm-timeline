...
// The constants and resolution helpers below are shared with the
// timeline's drop-into-strip handling (h3_mctx_ui.js imports them) --
// same trust rules everywhere: in-place selection when the content
// already lives in output, upload to dropped/ otherwise.
const NODES = ["H3LoadVideoWithMCtx", "H3LoadMCtx"];
export const SUBFOLDER = "dropped";
export const VIDEO_RE = /\.(mp4|mkv|webm|mov)$/i;
export const SIDECAR_SUFFIX = ".mctx.safetensors";
...
export async function uploadToOutput(file, overwrite, asName) {
...
export async function serverFileSize(value) {
...
export function isArtiusVideo(asset) {
