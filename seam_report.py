...
# ---- local jump where an arriving window starts ----------------------
# measure_cuts reads whole frames at 128x72, which is right for a cut and
# blind to this: a take that arrives slower (and so sharper) than the clip
# it lands in. The picture as a whole barely moves; one object takes a
# 4x larger step and the background turns from sharp to motion-blurred in
# a single frame, exactly where the take's own frames give way to its
# pinned window. It passes at generation size and shows once a refine has
# sharpened both sides (demo_looping clip_00006, 2026-09-19).
#
# So: the WORST blocks, each against its own typical change, and that
# number against the SAME-PHASE transitions one latent step (4 frames)
# either side -- the raw number rises with any accelerating shot, and
# H3's 4-frame cadence makes neighbouring transitions incomparable.
# Measured over the seven arriving takes of two folders: the one that
# pops 1.59, all others 0.81-0.98. One positive is a thin sample; the
# thresholds are a first guess and say so in the UI wording.
ARRIVAL_WIDTH = 960          # frames are scaled to this width to measure
ARRIVAL_BLOCK = 32
ARRIVAL_WORST = 16           # blocks averaged
ARRIVAL_PHASE = 4            # frames per latent step
ARRIVAL_BUMP, ARRIVAL_JUMP = 1.3, 1.8


def arrival_verdict(ratio):
    return ("clean" if ratio < ARRIVAL_BUMP else
            "local bump" if ratio < ARRIVAL_JUMP else "local jump")


def measure_arrival(clip_path, at):
    """Local jump INTO delivered frame `at`, where an arriving window starts.

    Returns {"ratio", "verdict", "frame"} or None when the take is too
    short around `at` to have same-phase neighbours on both sides.
    """
    import av
    import numpy as np
    at = int(at)
    if at < 1 + ARRIVAL_PHASE:
        return None
    # The WHOLE take is each block's baseline: measured against only the
    # second around the join, a busy arrival raises its own baseline and
    # the one bad take read 1.36 instead of 1.59 (others <= 1.07 vs 0.98).
    # Streamed -- only the previous frame is kept.
    b = ARRIVAL_BLOCK
    change, prev = [], None
    with av.open(clip_path) as container:
        ...
        w = ARRIVAL_WIDTH
        h = max(b, int(round(
            stream.height * w / float(stream.width) / 2.0)) * 2)
        rows, cols = h // b, w // b
        for frame in container.decode(video=0):
            cur = (frame.reformat(width=w, height=h, format="gray")
                   .to_ndarray().astype(np.float32))
            if prev is not None:
                d = np.abs(cur - prev)[:rows * b, :cols * b]
                change.append(d.reshape(rows, b, cols, b).mean(axis=(1, 3)))
            prev = cur
    if len(change) < at + ARRIVAL_PHASE:
        return None
    change = np.stack(change)
    k = at
    rel = (change / (np.median(change, axis=0) + 0.05)).reshape(
        len(change), -1)
    worst = np.sort(rel, axis=1)[:, -ARRIVAL_WORST:].mean(axis=1)
    j = k - 1                         # the transition INTO frame `at`
    near = [worst[i] for i in (j - ARRIVAL_PHASE, j + ARRIVAL_PHASE)
            if 0 <= i < len(worst)]
    if len(near) < 2 or not min(near):
        return None
    ratio = float(worst[j] / (sum(near) / len(near)))
    return {"ratio": round(ratio, 2), "verdict": arrival_verdict(ratio),
            "frame": at}


def arrival_frame(meta):
    """Delivered frame an after-pin's window starts at, or None."""
    covered = sum(int(s.get("source_frames", 0) or 0)
                  for s in mctx.parse_pins(meta) if s.get("place") == "after")
    if not covered:
        return None
    at = (int(meta.get("raw_frames", 0) or 0) - covered
          - int(meta.get("pinned_head_frames", 0) or 0))
