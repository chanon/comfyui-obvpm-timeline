...
WINDOW_CHOICES = ("39", "22", "5", "56", "90", "1")
...
def snap_run_nearest(n):
    """Closest 17k+5 clip length to n (never below the 5-frame base).

    snap_frames_up is the right rule when a length must COVER something;
    this one is for a length the user asked for directly, where rounding
    to whichever rung is nearer is what they meant. Ties go up.
    """
    n = max(FRAME_BASE, int(round(n)))
    lo = n - (n - FRAME_BASE) % FRAMES_PER_GROUP
    hi = lo + FRAMES_PER_GROUP
    return lo if (n - lo) * 2 < FRAMES_PER_GROUP else hi
